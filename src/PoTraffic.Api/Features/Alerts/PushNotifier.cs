...
{
    Task SendAsync(Guid userId, AlertDto alert, CancellationToken ct = default);
}
    ...
    ILogger<WebPushNotifier> logger) : IPushNotifier
{
    ...
    public async Task SendAsync(Guid userId, AlertDto alert, CancellationToken ct = default)
    {

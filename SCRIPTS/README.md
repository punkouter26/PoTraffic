...
| `post-deploy-smoke.ps1` | CI/CD rule #9 — runs three browser-style smoke checks against a freshly-deployed App Service instance: `/health` (dependency status), `/health/ready` (hydration complete), `GET /` (render-tree / Blazor bundle hash), `/diag/keyvault` (Key Vault + Managed Identity wiring, optional). Exits non-zero if any check fails. |
| `triage-50030.ps1` | Forensic triage for `HTTP Error 500.30 - ASP.NET Core app failed to start`. Captures App Service instance state, downloads + greps the application-log filesystem files for `500.30`, `HostingStartupException`, `AuthorizationPermissionMismatch`, `HydrationFailed`, lists storage RBAC for the target account, live-probes `/health`, `/health/ready`, `/`, and tails logs. **Read-only — never mutates the environment.** Use this when a deploy goes red. |

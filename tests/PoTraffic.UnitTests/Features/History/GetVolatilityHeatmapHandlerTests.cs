...
public sealed class GetVolatilityHeatmapHandlerTests
{
    ...
    [Fact]
    public async Task Heatmap_GroupsSamplesIntoOneCellPerWeekdayQuarterHour()
    {
        (TableStorageContext db, RouteId routeId, UserId userId) = SeedRoute();

        // Two samples share a cell and one does not: 12:00 UTC → 08:00 EDT and 12:05 UTC →
        // 08:05 EDT both fall in the 08:00 quarter, while 13:00 UTC → 09:00 EDT starts its own.
        AddPoll(db, routeId, TuesdayUtc.AddHours(12), 600);
        AddPoll(db, routeId, TuesdayUtc.AddHours(12).AddMinutes(5), 800);
        ...
        HeatmapCellDto eight = result.Cells.Single(c => c.Hour == 8 && c.Quarter == 0);
        ...
        HeatmapCellDto nine = result.Cells.Single(c => c.Hour == 9 && c.Quarter == 0);
        nine.SampleCount.Should().Be(1);
        nine.StdDevDurationSeconds.Should().Be(0, "a single sample has no spread");
    }

    [Fact]
    public async Task Heatmap_BucketingQuarterHourSlotsSeparatesMinutes15From45()
    {
        (TableStorageContext db, RouteId routeId, UserId userId) = SeedRoute();

        // 14:15 UTC = 10:15 EDT (second quarter), 14:45 UTC = 10:45 EDT (fourth).
        ...
        result.Cells.Should().HaveCount(2, "10:15 and 10:45 fall in different quarter-hour slots");
        result.Cells.Should().ContainSingle(c => c.Hour == 10 && c.Quarter == 1 && c.MeanDurationSeconds == 600);
        result.Cells.Should().ContainSingle(c => c.Hour == 10 && c.Quarter == 3 && c.MeanDurationSeconds == 900);
    }

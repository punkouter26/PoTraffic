...
/// </summary>
public sealed class RerouteAccuracyTests
{
    ...
    // The per-case [Theory] that used to live here fed the same 20 rows through the same
    // handler and asserted each one independently — 20 tests that duplicated, case by case,
    // exactly what the aggregate test below already proves. It also could not measure what
    // SC-004 actually claims: accuracy is a ratio over the sequence, and a per-case theory
    // reports "20 passed", never "19 of 20".

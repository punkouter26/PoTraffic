...
public sealed class CheckNowScenarios : PlaywrightTestBase
{
    ...
    public async Task CheckNow_ClickButton_ShowsNotificationWithFeedback()
    {
        ...
        (RouteId routeId, string origin, string destination) = await api.SeedRouteAsync(
        ...
        try
        {
            await probeButton.First.WaitForAsync(new() { Timeout = 20_000, State = WaitForSelectorState.Visible });
        }
        catch (Exception ex)
        {
        ...
        await probeButton.First.ClickAsync();
